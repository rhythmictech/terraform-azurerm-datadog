# Datadog `main` logs index management. Cloud-agnostic (datadog provider only);
# ported ~verbatim from the AWS module.
#
# v4 notes: `name` is force-new (a rename recreates the index); `filter` (with
# nested `query`) and `name` are required. `disable_daily_limit` is set from
# whether a daily limit was supplied.
