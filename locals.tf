locals {
  ...
  # Null on the secretless path BEFORE the create-path branch: with
  # secretless_auth_enabled the password resource has count = 0, so indexing
  # [0] must never be reached.
  datadog_client_secret = var.secretless_auth_enabled ? null : (local.create ? azuread_application_password.this[0].value : var.datadog_client_secret)
