# Datadog estimated-usage anomaly / threshold monitors. Cloud-agnostic (datadog
# provider only); ported ~verbatim from the AWS module.
#
# v4 audit: the AWS source used neither the removed `locked` attribute nor the
# changed `restricted_roles` behavior, so the port is clean. Core args (`type`,
# `name`, `query`, `message`, `monitor_thresholds`, `scheduling_options`) are
# unchanged in v4.
locals {
  metric_map = {
    "hosts"         = "hosts"
...
# County things like hosts should probably be split from summy things like logs.
